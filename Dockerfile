...
RUN set -e \
  ...
  ; apteryx git='1:2.47.0-0ppa2~ubuntu22.04.1'
RUN set -e \
...
RUN set -e \
  ...
  ; sh rust.sh -y --default-toolchain '1.82.0' \
  ; rm rust.sh
RUN set -e \
  ; mkdir -p /exports/root/ \
...
ENV \
  ...
  GO111MODULE=auto
RUN set -e \
  ; clone --https --shallow --ref=652e24f54803012def412a67109768a59645a131 https://github.com/jmbaur/gosee \
...
COPY --from=unzip /exports/ /
RUN set -e \
  ; wget -O /tmp/bun.zip https://github.com/oven-sh/bun/releases/download/bun-v1.1.34/bun-linux-x64.zip \
  ; mkdir /tmp/bun \
  ; cd /tmp/bun \
  ; unzip /tmp/bun.zip \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/autotag.tgz "https://github.com/pantheon-systems/autotag/releases/download/v1.3.31/autotag_linux_amd64.tar.gz" \
