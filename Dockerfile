...
RUN set -e \
  ; clone --https --tag='v1.86.8' https://github.com/stayradiated/dotfiles \
...
RUN set -e \
  ...
  ; sh rust.sh -y --default-toolchain '1.70.0' \
  ; rm rust.sh
RUN set -e \
  ; mkdir -p /exports/root/ \
...
COPY --from=python3-pip /exports/ /
RUN set -e \
  ; wget -O /tmp/nvim.appimage 'https://github.com/neovim/neovim/releases/download/v0.9.1/nvim.appimage' \
...
ENV \
  ...
  GO111MODULE=auto
RUN set -e \
  ; clone --https --shallow --ref=2c6930b9012e7ecaaaed5afbb2a0fa0103f2562f https://github.com/jmbaur/gosee \
...
RUN set -e \
  ; wget -O /tmp/bun.zip https://github.com/oven-sh/bun/releases/download/bun-v0.6.7/bun-linux-x64.zip \
  ; mkdir /tmp/bun \
  ; cd /tmp/bun \
  ; unzip /tmp/bun.zip \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; apteryx zeal='1:0.6.1-1.2'
RUN set -e \
  ; mkdir -p /exports/usr/bin/ /exports/usr/share/man/man1/ \
  ; mv /usr/bin/zeal /exports/usr/bin/ \
  ; mv /usr/share/man/man1/update-mime-database.1.gz /exports/usr/share/man/man1/

# ZOXIDE
FROM base AS zoxide
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/zoxide.tar.gz "https://github.com/ajeetdsouza/zoxide/releases/download/v0.9.1/zoxide-0.9.1-x86_64-unknown-linux-musl.tar.gz" \
  ; mkdir -p /tmp/zoxide \
...
RUN set -e \
  ...
  ; mv /usr/share/man/man1/exa_colors.5 /usr/share/man/man1/exa.1 /usr/share/man/man1/unzip.1.gz /exports/usr/share/man/man1/ \
