...
RUN set -e \
  ...
  ; apteryx git='1:2.41.0-0ppa1~ubuntu22.04.1'
RUN set -e \
...
RUN set -e \
  ; wget -O /tmp/go.tgz "https://dl.google.com/go/go1.20.5.linux-amd64.tar.gz" \
  ; tar xzvf /tmp/go.tgz \
  ; mv go /usr/local/go \
  ; rm -rf /tmp/go.tgz
RUN set -e \
...
RUN set -e \
  ; clone --https --tag='v1.87.4' https://github.com/stayradiated/dotfiles \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget "https://raw.githubusercontent.com/tj/n/v9.1.0/bin/n" -O /usr/local/bin/n \
...
RUN set -e \
  ...
  ; pip3 install wheel \
  ; python3 -m pip install -U pip==23.1.2
RUN set -e \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; apteryx cmake='3.22.1-1ubuntu1.22.04.1'
RUN set -e \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; apteryx binutils='2.38-4ubuntu2.2' \
...
COPY --from=clone /exports/ /
RUN set -e \
  ; clone --https --shallow --tag v1.16.2 github.com/strukturag/libheif \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; apteryx python2.7='2.7.18-13ubuntu1.1' \
...
COPY --from=clone /exports/ /
RUN set -e \
  ; clone --https --tag='0.42.0' https://github.com/junegunn/fzf \
...
RUN set -e \
  ; wget -O /tmp/firefox.tar.bz2 https://download-installer.cdn.mozilla.net/pub/firefox/releases/115.0.2/linux-x86_64/en-US/firefox-115.0.2.tar.bz2 \
  ; cd /opt \
...
RUN set -e \
  ...
  ; apt-get update \
  ; apteryx google-chrome-beta='115.0.5790.75-*'
RUN set -e \
...
RUN set -e \
  ...
  ; mkdir -p /usr/local/ruby/3.2.2 \
  ; ./bin/ruby-build --verbose 3.2.2 /usr/local/ruby/3.2.2 \
  ; ln -s /usr/local/ruby/3.2.2/bin/bundle   /usr/local/bin/bundle \
  ; ln -s /usr/local/ruby/3.2.2/bin/gem      /usr/local/bin/gem \
  ; ln -s /usr/local/ruby/3.2.2/bin/rake     /usr/local/bin/rake \
  ; ln -s /usr/local/ruby/3.2.2/bin/ruby     /usr/local/bin/ruby \
  ; apt-get remove --purge -y $packages \
  ; apt-get -q clean \
  ; rm -rf /root/src
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ /exports/usr/local/ \
  ; mv /usr/local/bin/bundle /usr/local/bin/gem /usr/local/bin/rake /usr/local/bin/ruby /exports/usr/local/bin/ \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/charles.tgz 'https://www.charlesproxy.com/assets/release/4.6.4/charles-proxy-4.6.4_amd64.tar.gz' \
...
ENV \
  ...
  GO111MODULE=auto
RUN set -e \
  ; clone --https --shallow --ref=bd3dbedf4b1b056ef58776696bfb38f575ec764f https://github.com/jmbaur/gosee \
...
RUN set -e \
  ; wget -O /tmp/bun.zip https://github.com/oven-sh/bun/releases/download/bun-v0.6.13/bun-linux-x64.zip \
  ; mkdir /tmp/bun \
  ; cd /tmp/bun \
  ; unzip /tmp/bun.zip \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /usr/local/bin/fastgron "https://github.com/adamritter/fastgron/releases/download/v0.6.4/fastgron-ubuntu" \
...
RUN set -e \
  ...
  ; apt update \
  ; apteryx brave-browser-beta='1.57.1*'
RUN set -e \
...
ENV \
  ...
  PIPX_BIN_DIR=/usr/local/bin
RUN set -e \
  ; pipx install sqlite-utils=='3.33'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ /exports/usr/local/ \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; wget -O /tmp/obsidian.deb "https://github.com/obsidianmd/obsidian-releases/releases/download/v1.3.5/obsidian_1.3.5_amd64.deb" \
...
ENV \
  ...
  GO111MODULE=auto
RUN set -e \
  ; clone --https --tag=v1.4.0 https://github.com/stayradiated/wifi \
...
RUN set -e \
  ...
  ; clone --https --ref='d717f1c336d44f7d87bb09a5dc5d124673345304' https://github.com/haikarainen/light \
...
RUN set -e \
  ...
  ; wget -O /tmp/feh.tar.bz2 https://feh.finalrewind.org/feh-3.10.tar.bz2 \
...
RUN set -e \
  ...
  ; apt-get -q update \
  ; apt-get install -y --no-install-recommends --auto-remove $packages \
  ; clone --https --tag='v0.12.2' https://github.com/alacritty/alacritty \
...
COPY --from=node /exports/ /
RUN set -e \
  ; npm install -g '@stayradiated/shoebox@2.4.0'
RUN set -e \
...
COPY --from=node /exports/ /
RUN set -e \
  ; npm install -g 'npm-check-updates@16.10.15'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ /exports/usr/local/lib/node_modules/ \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; wget -O /tmp/miller.deb https://github.com/johnkerl/miller/releases/download/v6.8.0/miller-6.8.0-linux-amd64.deb \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/gh.tgz 'https://github.com/cli/cli/releases/download/v2.31.0/gh_2.31.0_linux_amd64.tar.gz' \
  ; tar xzvf /tmp/gh.tgz \
  ; rm /tmp/gh.tgz \
  ; mv 'gh_2.31.0_linux_amd64/bin/gh' /usr/local/bin/gh \
  ; rm -r 'gh_2.31.0_linux_amd64'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/fd.tgz 'https://github.com/sharkdp/fd/releases/download/v8.7.0/fd-v8.7.0-x86_64-unknown-linux-musl.tar.gz' \
  ; tar -xzvf /tmp/fd.tgz \
  ; rm /tmp/fd.tgz \
  ; mv 'fd-v8.7.0-x86_64-unknown-linux-musl' fd \
...
RUN set -e \
  ...
  ; wget -O /usr/local/lib/docker/cli-plugins/docker-compose 'https://github.com/docker/compose/releases/download/v2.20.0/docker-compose-linux-x86_64' \
...
RUN set -e \
  ...
  ; apt-get update \
  ; apteryx docker-ce-cli='5:24.0.4*'
RUN set -e \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O bat.tgz 'https://github.com/sharkdp/bat/releases/download/v0.23.0/bat-v0.23.0-x86_64-unknown-linux-gnu.tar.gz' \
  ; tar -xzvf bat.tgz \
  ; rm bat.tgz \
  ; mv 'bat-v0.23.0-x86_64-unknown-linux-gnu/bat' /usr/local/bin/bat \
  ; rm -rf 'bat-v0.23.0-x86_64-unknown-linux-gnu'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ \
