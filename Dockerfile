...
ENV \
  ...
  PIPX_BIN_DIR=/usr/local/bin
RUN \
  pipx install eyeD3=='v0.9.6'
RUN \
  mkdir -p /exports/usr/local/bin/ /exports/usr/local/ && \
  mv /usr/local/bin/eyeD3 /exports/usr/local/bin/ && \
  mv /usr/local/pipx /exports/usr/local/
...
ENV \
  ...
  PIPX_BIN_DIR=/usr/local/bin
RUN \
  pipx install beets=='1.4.9'
RUN \
  mkdir -p /exports/usr/local/bin/ /exports/usr/local/ && \
  mv /usr/local/bin/beet /exports/usr/local/bin/ && \
  mv /usr/local/pipx /exports/usr/local/
...
# SHELL-BEETS
FROM shell-admin AS shell-beets
COPY --from=make /exports/ /
COPY --from=beets /exports/ /
COPY --from=eyed3 /exports/ /
RUN \
  cd dotfiles && \
  make beets
RUN \
  mkdir -p /home/admin/exports/home/admin/.config/beets/ && \
  mv /home/admin/.config/beets/config.yaml /home/admin/exports/home/admin/.config/beets/
USER root
RUN \
  mkdir -p /exports/usr/local/bin/ /exports/usr/local/ && \
  mv /usr/local/bin/beet /usr/local/bin/eyeD3 /exports/usr/local/bin/ && \
  mv /usr/local/pipx /exports/usr/local/
...
COPY --from=shell-beets --chown=admin /home/admin/exports/ /
COPY --from=shell-beets /exports/ /
