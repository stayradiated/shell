...
RUN set -e \
  ; wget -O /tmp/go.tgz "https://dl.google.com/go/go1.21.0.linux-amd64.tar.gz" \
  ; tar xzvf /tmp/go.tgz \
  ; mv go /usr/local/go \
  ; rm -rf /tmp/go.tgz
RUN set -e \
...
RUN set -e \
  ...
  ; pip3 install wheel \
  ; python3 -m pip install -U pip==23.2.1
RUN set -e \
...
RUN set -e \
  ...
  ; sh rust.sh -y --default-toolchain '1.71.0' \
  ; rm rust.sh
RUN set -e \
  ; mkdir -p /exports/root/ \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; apteryx binutils='2.38-4ubuntu2.3' \
...
RUN set -e \
  ; wget -O /tmp/firefox.tar.bz2 https://download-installer.cdn.mozilla.net/pub/firefox/releases/116.0.2/linux-x86_64/en-US/firefox-116.0.2.tar.bz2 \
  ; cd /opt \
...
COPY --from=unzip /exports/ /
RUN set -e \
  ; wget -O /tmp/bun.zip https://github.com/oven-sh/bun/releases/download/bun-v0.7.3/bun-linux-x64.zip \
  ; mkdir /tmp/bun \
  ; cd /tmp/bun \
  ; unzip /tmp/bun.zip \
...
ENV \
  ...
  PIPX_BIN_DIR=/usr/local/bin
RUN set -e \
  ; pipx install sqlite-utils=='3.34'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ /exports/usr/local/ \
...
COPY --from=apteryx /exports/ /
RUN set -e \
  ; wget -O /tmp/obsidian.deb "https://github.com/obsidianmd/obsidian-releases/releases/download/v1.3.7/obsidian_1.3.7_amd64.deb" \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/caddy.tgz 'https://github.com/caddyserver/caddy/releases/download/v2.7.3/caddy_2.7.3_linux_amd64.tar.gz' \
...
COPY --from=node /exports/ /
RUN set -e \
  ; npm install -g '@stayradiated/shoebox@2.5.0'
RUN set -e \
...
COPY --from=wget /exports/ /
RUN set -e \
  ; wget -O /tmp/gh.tgz 'https://github.com/cli/cli/releases/download/v2.32.1/gh_2.32.1_linux_amd64.tar.gz' \
  ; tar xzvf /tmp/gh.tgz \
  ; rm /tmp/gh.tgz \
  ; mv 'gh_2.32.1_linux_amd64/bin/gh' /usr/local/bin/gh \
  ; rm -r 'gh_2.32.1_linux_amd64'
RUN set -e \
  ; mkdir -p /exports/usr/local/bin/ \
...
RUN set -e \
  ...
  ; apt-get update \
  ; apteryx docker-ce-cli='5:24.0.5*'
RUN set -e \
